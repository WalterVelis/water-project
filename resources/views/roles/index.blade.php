              ...
              <div class="card-body">
                ...
                <div class="row d-none">
                  <div class="col-12 text-left">
                      <a id="exportXlsx" href="{{ route('role_xlsx') }}" class="btn btn-sm btn-rose">{{ __('Export xlsx') }}</a>
                      <a id="exportCsv" href="{{ route('role_csv') }}" class="btn btn-sm btn-rose">{{ __('Export csv') }}</a>
                      <a id="exportPdf" href="{{ route('rolePdf',['all'])}}" target='_blank' class="btn btn-sm btn-rose">{{ __('Export pdf') }}</a>
                  </div>
                </div>

                <div class="row col-12">
                  <div class="col-lg-11 col-md-11 col-sm-11 col-xs-12 text-right"> 
                   <a href="{{ route('role.create') }}" class="btn-add"><img src="{{asset("img/icons").'/Agregar.png'}}"></a>
                  </div>
                  ...
                  <table id="datatables" class="table table-striped table-no-bordered table-hover" style="display:none">
                    ...
                    <tbody>
                      @foreach($roles as $role)
                        <tr>
                          <td>
                           <a href="{{URL::action('RoleController@show',$role->id)}}">{{ $role->name }}</a> 
                          </td>
                          <td>
                            {{ $role->description }}
                          </td>
